...
function pageParser(job, result) {
  ...
  parserResult.parser1FinishedAt = Date.now();
  return parserResult;
  // });
}
...
function createProcessQueuedURLs(executionDoc, fn) {
  ...
  queue.process('execution:' + executionDoc._id, executionDoc.maxParallelRequests, function (job, done) {
    ...
    // const nightInst =
    function* nightmareWithGenerator(jobForNightmare, jobDone) {
      ...
      let result = {};
      yield nightmareBrowser.goto(job.data.URL).then(function (rez) {
      ...
      yield nightmareBrowser.wait(job.data.waitForSelector)
        .evaluate(function () {
    ...
    vo(nightmareWithGenerator(job, done))(function (err, result) {
      if (err) done(error);
...
function queStartingExecutionURLs(executionDoc, fn) {
  ...
  _.forEach(executionDoc.startingLinks, function (startingLink) {
    ...
    const job = queue.create('execution:' + executionDoc._id, {
      title: 'crawl:' + executionDoc.crawlerCustomId + ':url:' + startingLink.url,
      // user: 1,
