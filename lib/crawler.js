    ...
    .on('job complete', function (id, result) {
      kue.Job.get(id, function (err, job) {
        ...
        _.forEach(job.data.parser, function (toParse) {
          for (let selectorName in toParse) {
            if (selectorName == 'path') {
              result[selectorName] = '>>';
              result[selectorName] += $(toParse[selectorName]).map(function (i, el) {
                // this === el
                return $(this).text();
              }).get().join('>>');
            } else if (selectorName == 'name' || selectorName == 'sku' || selectorName == 'price') {
              result[selectorName] = $(toParse[selectorName]).first().text();
            } else if (selectorName == 'image') {
              result[selectorName] = $(toParse[selectorName]).first().attr('src');
            } else if (selectorName == 'description') {
              result[selectorName] = $(toParse[selectorName]).first().html();
            } else if (selectorName == 'productId' || selectorName == 'groupId' || selectorName == 'currency') {
              continue;
              // TODO: ifs for productId, groupId, currency
            }
          }
        });
        // TODO: inser the crawled page data to DB
