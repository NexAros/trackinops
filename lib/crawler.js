...
Crawler.prototype.startPreProcessing = function (crawler, fn) {
  ...
  this.createExecutionDocument(crawler, function (err, executionDoc) {
    ...
    return preProcessCrawlerURLs('START', executionDoc, fn);
...
function preProcessCrawlerURLs(type = 'START', executionDoc, fn) {
  fn = fn || noop;
  switch (type) {
    case 'START': executionDoc.startingLinks.forEach(function (link) {
      // console.log(link);
      if (link.key === 'START') {
        // TODO: pre-check links if they are loadable (correct url; not 301; etc.)

        // creates Redis crawl que for each starting link
        let job = queue.create('execution:' + executionDoc._id, {
          ...
          // if (err) return
          if (err) console.log('error job ' + job.id, err);
